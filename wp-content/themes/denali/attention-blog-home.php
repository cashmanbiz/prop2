<?php
/**
 * Attention Blog Home displays the attenion grabbing element on the homage page.
 *
 *
 ...
 */
  ...
  if(!current_theme_supports('post_page_attention_grabber_area')) {
    return;
  }

  if(get_post_meta($post->ID, 'hide_header', true) == 'true') {
    denali_theme::console_log('AG: Blog Header not rendered, disabled in post settings.');
    return;
  }

  if($ds['post_page_attention_grabber_area_hide'] == 'true') {
    denali_theme::console_log('AG: Header not rendered, Home Page Attention Grabber is disabled in Theme Options.');
    return;
  }

  $this_widget_area = 'post_page_attention_grabber';
  ...
  if(count($tabs) > 1) {
    denali_theme::console_log('AG: Rendering Blog Home Tabbed attention grabber.');
    $multi_tab = true;
  } else {
    denali_theme::console_log('AG: Rendering Blog Home Untabbed attention grabber.');
  }
